...
def get_scriptable_transforms():
    audio_transforms = list(filter(lambda x: (type(getattr(transforms, x)) == type) and (not x in ['AudioTransform', 'ComposeAudioTransform']), dir(transforms)))
    audio_transforms = [getattr(transforms, x) for x in audio_transforms]
    audio_transforms = list(filter(lambda x: issubclass(x, AudioTransform), audio_transforms))
...
def test_inversion(test_files, transform: AudioTransform):
    ...
    for i in range(raw.size(0)):
        ...
        for k, v in x_inv.items():
            if v.ndim == 1:
                v = v.unsqueeze(0)
            torchaudio.save(f"test/reconstructions/{names[i]}_{transform_name}_{k}.wav", v, sample_rate=44100)
    return True
...
def test_scriptable(transform_type):
    ...
    transform_type.test_scripted_transform(scripted_transform)
    return True
...
def test_combinations(test_files, transform):
    ...
    for i in range(raw.size(0)):
        ...
        if transform.invertible:
            ...
            torchaudio.save(f"test/reconstructions/{names[i]}_{transform_name}.wav", x_inv, sample_rate=44100)
    return True